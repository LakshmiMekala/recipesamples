...
function testcase1 {
mashling-gateway -c examples/recipes/v2/simple-flogo-flow-pattern.json > /tmp/rest1.log 2>&1 & pId=$!
sleep 15
...
function testcase2 {
mashling-gateway -c examples/recipes/v2/simple-flogo-flow-pattern.json > /tmp/rest2.log 2>&1 & pId=$!
sleep 15
...
function testcase3 {
mashling-gateway -c examples/recipes/v2/simple-flogo-flow-pattern.json > /tmp/rest3.log 2>&1 & pId=$!
sleep 15
